...
3) Updating backup with new changes:
	3.1) Updates add new changes from "target_dir"
	3.2) The backup_loc retains old backup history. So it's possible to restore to a specific version of the backup.
	3.3) The updates can be done in either differential or incremental fashion. They can't be mixed.

	3.4) create an incremental backup of "target_dir". Incremental backups can be made only after an intiail full backup.
			python .\enky_backup_checksum.py backup -bc -bi -bl backkup_loc -bt target_dir

	3.5) create a differential backup of "target_dir".
...
6) Creating checksums of a directory
	6.1) This will recursively examine all files within the "hash_target" directory and store checksums in a "hash_file"
	6.2) The goal is to detect bit rot and silent data corruption of files
	6.3) The command:
		python .\enky_backup_checksum.py checksum -hs -ht hash_target -hf hash_file 

7) Checking file checksums
	7.1) This will verify the checksums of all files within the "hash_target" that have a hash previously created in "hash_file"
	7.2) Files that got changed/corrupted and deleted will be detected. Newly added files will not be discovered.
	7.3) The command:
